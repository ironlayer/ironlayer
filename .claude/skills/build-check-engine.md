...
├── ironlayer-core/                     (existing Python package — UNCHANGED)
│   ├── core_engine/                    (existing Python modules)
...
| Python Type | Rust Mirror | Critical Fields |
|---|---|---|
| `ModelKind` | `enum ModelKind` | `FULL_REFRESH`, `INCREMENTAL_BY_TIME_RANGE`, `APPEND_ONLY`, `MERGE_BY_KEY` |
| `Materialization` | `enum Materialization` | `TABLE`, `VIEW`, `MERGE`, `INSERT_OVERWRITE` |
| `Dialect` | `enum Dialect` | `Databricks`, `DuckDB`, `Redshift` (serializes lowercase) |
| `ViolationSeverity` | `enum Severity` | `Error` → BREAKING, `Warning` → WARNING, `Info` → INFO |
| `SchemaContractMode` | `enum SchemaContractMode` | `DISABLED`, `WARN`, `STRICT` |
| `SQLGuardViolation` | maps to SAF rules | 11 `DangerousOperation` variants |
| `_REF_PATTERN` regex | Rust `regex` crate | `\{\{\s*ref\s*\(\s*(?:'([^']+)'\|"([^"]+)")\s*\)\s*\}\}` |
| `_REQUIRED_FIELDS` | hardcoded set | `{"name", "kind"}` |
| `_KNOWN_FIELDS` | hardcoded set | 13 known header fields |

**CLI patterns to replicate exactly:**
- Typer command with `repo: Path` argument (exists=True, file_okay=False, resolve_path=True)
- `--json` / `--no-json` global flag
- Exit codes: 0 = success, 1 = failures found, 3 = internal error
- Metrics emission via `_emit_metrics()`
- Cloud upsell when not authenticated
...
`ironlayer-core` switches from `poetry-core` (pure-Python, `py3-none-any` wheels) to `maturin` (Rust+Python, platform-specific wheels). Key implications:
- `pyproject.toml` build-system changes from `poetry-core` to `maturin>=1.7,<2.0`
- `[tool.poetry]` sections replaced with `[tool.maturin]` configuration
- Resulting wheel changes from `py3-none-any` to platform-specific (e.g., `cp311-abi3-manylinux_2_17_x86_64`)
- `ironlayer` (CLI package) remains pure-Python with poetry-core
...
11. Integration: `cli/app.py` check command wired up with Typer
